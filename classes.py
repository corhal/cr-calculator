...
class Quest(object):
    def __init__(self, ident, name, chapter, quest_chain,
        ...
        self.locked = True
        self.full_conditions = self.__make_full_conditions()
    ...
    def __make_full_conditions(self):
        full_conditions = {}
        for item in self.item_conditions.keys():
...
class Player(object):
    ...
    def play_chapter(self, chapter_index):
        ...
        result = self.choose_quest()
        #print("--- {} seconds to choose and play {} ---".format(time.time() - start_time, chapter_index))
        return result
    ...
    def choose_quest(self): # returns tuple (missions_completed, day)
        ...
        while True:
            ...
            if len(available_quests) > 0:
                ...
                #print("--- {} seconds to choose and play {} ---".format(time.time() - start_time, quest.name))
    ...
    def play_quest(self, quest):
        ...
        #print(quest.full_conditions)
        start_time = time.time()
        for item in full_conditions.keys():
            start_time2 = time.time()
            self.farm_item(item, full_conditions[item])
            #print("--- {} seconds to farm {} {} for {} ---".format(time.time() - start_time2, item.name, full_conditions[item], quest.name))
    ...
    def farm_item(self, item, amount):
        ...
        while self.inventory.get(item, 0) < amount: # here
            #start_time2 = time.time()
            self.play_mission(farm_mission)
        print("--- {} seconds to farm {} ---".format(time.time() - start_time, item.name))
